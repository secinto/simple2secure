...
from src.db.database import db, TestSequence, Test
...
def get_sequence_from_url(url_query, app):
    ...
    for task in tasks:
        ...
        if test:
            ...
            response_text = "Some of the tasks from the sequence are not included in the services.json config. To be scheduled in the sequence they have to be included in services.json!"
        
        
        
    sequence_hash = file_utils.check_md5(json.dumps(sequence_content))
        
    test_sequence = TestSequence(sequence_name, json.dumps(sequence_content), sequence_hash, current_milli_time, app.config['POD_ID'])
