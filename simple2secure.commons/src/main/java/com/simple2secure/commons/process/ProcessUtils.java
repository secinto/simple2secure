...
public class ProcessUtils {
	 ...
	 * @return The {@link ProcessContainer} which wraps the relevant streams and processes.
	 */
	 ...
	 *          Specifies if the system environment variables should be cleared or not.
	 * @param executable
	 *          The string which should be used to start a process via ProcessBuilder.
	 * @return The {@link ProcessContainer} which wraps the relevant streams and processes.
	 */

	public static ProcessContainer createProcess(Map<String, String> environment, boolean cleanEnvironment, String... executable) {
		try {
	...
	/**
	 * Starts a process using powershell and Start-Process cmdlet. The provided executable string is used as command in the shell. </br>
	 * The provided executable string must be of the form <code>%PR_INSTALL% //MODE//%SERIVCE_NAME%</code>. </br>
	 * The %PR_INSTALL% must be the fully qualified file name of the prunsrv.exe file. If the %PR_INSTALL% location is set in the environment
	 * variable it is automatically take from there</br>
	 * The MODE is a string of the ones defined for prunsrv.exe. Usual modes are </br>
	 * <ul>
	 * <il><code>IS</code> for install service.</il> <il><code>DS</code> for delete service.</il> <il><code>ES</code> for execute (start)
	 * service.</il> <il><code>SS</code> for stop service.</il>.
	 * </ul>
	 * </br>
	 * </br>
	 * For instance providing an executable string <code>%PR_INSTALL //IS//%SERVICE_NAME%</code> would install a service which is defined via
	 * the environment variable SERVICE_NAME using the prunsrv.exe which is identified via PR_INSTALL. </br>
	 * Another way would be to provide the following executable string <code>D:/daemon-location/prunsrv.exe //DS//My Service</code>. For
	 * further information on the different modes and the usage of prunserv.exe see
	 * <a href= "https://commons.apache.org/proper/commons-daemon/procrun.html">Apache Commons Daemon</a> The provided environment variables
	 * are used to obtain the arguments for the command. The arguments are supplied using powershell ArgumentList. If successful a
	 * {@link ProcessContainer} is returned which contains the process itself and a the associated {@link ProcessStreamObservable}.
	 *
	 * @param executable
	 *          The string which is used as command in the console.
	 * @param runAsAdmin
	 *          If set to true the powershell is executed as Administrator rights.
	 * @param environment
	 *          The environment variables which are to be used in the console.
	 * @return The {@link ProcessContainer} which wraps the relevant streams, processes and consumers, if the 3 parts of the executable are
	 *         supplied correctly.
	 */
	public static ProcessContainer manageServiceWindows(String executable, boolean runAsAdmin, Map<String, String> environment) {
		...
		if (execParts.length == 3) {
			...
			if (runAsAdmin) {
				return createProcess("powershell.exe", "\"Start-Process -FilePath " + installProcess + " -ArgumentList \\\"`\\\"//" + mode + "//"
						+ serviceName + "`\\\"\\\", " + createCmdLineArgumentList(environment) + " -verb RunAs\"");
			} else {
				return createProcess("powershell.exe",
						"\"Start-Process -FilePath " + installProcess + " -ArgumentList \\\"`\\\"//" + mode + "//" + serviceName + "`\\\"\\\"");
			}
		}
		return null;
	}
	...
	/**
	 * Returns the absolute Java executable file name if available in the system. If the environment is setup only <code>java</code> is
	 * returned since it can be used to launch a java application.
	 *
	 * @return The absolute file name of the Java executable.
	 * @throws FileNotFoundException
	 * @throws InterruptedException
	 */
	public synchronized static String getJavaExecutable() throws FileNotFoundException, InterruptedException {
		...
		/*
		 * Unfortunately the environment is not set up, thus we try to find the java executable using other means.
		 */
	...
	/**
	 * Invokes a Java process using the provided parameters. Only the parameters are required as input, such as the following list of string
	 * arguments. <code>-cp, classpath.lib.jar, -Dsomeparam, startclass.start, input-param1</code> The Java executable is obtained
	 * automatically if available in the system and supplied to invoke the java process.
	 *
	 * @param arguments
	 ...
	 * Invokes a Java process using the provided parameters. Only the parameters are required as input, such as the following list of string
	 * arguments. <code>-cp, classpath.lib.jar, -Dsomeparam, startclass.start, input-param1</code> The Java executable is obtained
	 * automatically if available in the system and supplied to invoke the java process. The provided environment and the cleanEnv parameters
	 * are used to build the execution environment. See {@link #createProcess(String, Map, boolean)} for further details.
	 *
	 * @param cmdArray
	 *          The list of parameters which should be applied as input to the Java process.
	 * @param environment
	 *          The environment variables used to setup the Java environment.
	 * @param cleanEnv
	...
	/**
	 * Creates a process executing the provided executable string using a console, CMD in Windows and SH in Linux based systems. The provided
	 * environment variables are added to the current environment setting. If the provided map contains an already existing variable, it is
	 * overwritten. If cleanEnv is set to true, the previously existing environment variables are cleared.
	 *
	 * @param executable
	 *          The executable string which is executed in an console.
	 * @param environment
	 *          The environment variables to use.
	 * @param cleanEnv
	 *          Specifies if the existing environment variables are cleared or not.
	 * @return The {@link ProcessContainer} which wraps the relevant streams, processes and consumers.
	 */
	public static ProcessContainer createConsoleProcess(String executable, Map<String, String> environment, boolean cleanEnv) {
		if (isWindows) {
