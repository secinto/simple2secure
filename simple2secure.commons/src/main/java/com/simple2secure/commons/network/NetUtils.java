...
public class NetUtils {
	...
	/**
	 * Converts an IP address of the format [xx]x.[xx]x.[xx]x.[xx]x to an integer. Throws an exception if the format of the String is faulty.
	 *
	 * @param ipAddr
		...
		for (int i = 0; i <= ipAddr.length(); i++) {
			if (i == ipAddr.length() || ipAddr.charAt(i) == '.') {
				if (i < ipAddr.length() && ++dots == 4) {
					log.debug("Too many dots in " + ipAddr);
					return 0;
				}
				if (ipOctet < 0 || ipOctet > 255) {
					log.debug("Wrong IP values in " + ipAddr);
					return 0;
				}
				...
				log.debug("Wrong char in IP address " + ipAddr);
				return 0;
			}
		}

		if (dots != 3) {
			log.debug("IP address too short " + ipAddr);
			return 0;
		}
		return ipInt;
	}
	...
	/**
	 * Converts the <code>Integer</code> representation of an IP address into a <code>String</code> representation.
	 *
	 * @param ipAddr
	...
	/**
	 * Converts the provided byte array values into an IPv4 address in <code>String</code> representation (X[XX].X[XX].X[XX].X[XX]).
	 *
	 * @param address
	 *          The byte array containing the IP address values.
	 * @param offset
	 *          The starting point of the address in the provided byte array. From this index 4 bytes are taken for conversion into the IP
	 *          address.
	...
	/**
	 * Converts the provided byte array values into an IPv6 address in <code>String</code> representation
	...
	/**
	 * Converts a signed byte value into a usinged decimal string representation.
	 *
	 * @param b
	 *          The signed byte value which should be converted.
	 * @return The unsigned <code>String</code> representative of the provided signed byte value.
	 */
	 ...
	 * @param off
	 *          the offset indicating the start position within the given byte array; the following <code>len</code> bytes will be converted
	...
	/**
	 * Converts the specified subsequence of the given byte array into a hexadecimal string of the format:
	 * 01'delimiter'23'delimiter'34'delimiter'56... . Whereas for the delimiter any arbitrary <code>String</code> can be used which separates
	 * the entries of the byte array.
	 *
	 * @param byteArray
	 *          the byte array containing the data
	 * @param off
	 ...
	 *          how many bytes shall be written
	 * @param delimiter
	 *          the delimiter to be used for separating the hex values, e.g. ":" (specify "" for not using a delimiter)
	 * @param linebreak
	 *          Specifies the amount of bytes should be printed in one line. After the specified amount of bytes a linebreak is inserted.
	 * @return the string representation
	 */
	...
	/**
	 * Converts a byte array into a hexadecimal string of the format: 01'delimiter'23'delimiter'34'delimiter'56... . Whereas for the delimiter
	 * any arbitrary <code>String</code> can be used which separates the entries of the byte array.
	 *
	 * @param byteArray
	 *          the byte array containing the data
	 * @param delimiter
	...
	/**
	 * Converts an int into a hexadecimal string. After each byte of the integer the provided delimiter is inserted. Therefore an integer can
	 * be converted into either an MAC address of format (XX:XX:XX:XX:XX:XX) or into an IP address of format (X[XX].X[XX].X[XX].X[XX]).
	 *
	 * @param i
	 *          The integer which should be converted into an hexadecimal string.
	 * @param delimiter
	...
	/**
	 * Converts an long into a hexadecimal string of the format XX:XX:XX:XX:XX:XX.
	 */

	/**
	 * Converts a long into a hexadecimal string of the format: XX:XX:.. . Where X can be 0 - 9 or A - F.s Uses big endian byte order.
	 *
	 * @param l
	 *          the long to convert
	 * @return the int as hexadecimal string representation
	...
	/**
	 * Converts a long into a hexadecimal string of the format: XX[delimiter]XX[delimiter].. . Where X can be 0 - 9 or A - F.s Uses big endian
	 * byte order.
	 *
	 * @param l
	 *          the long to convert
	...
	/**
	 * Converts the provided short value into hexadecimal values represented as String.
	 *
	 * @param s
	 *          The short value which should be transformed.
	...
	/**
	 * Returns the HEX representation of the provided short value as String. The provided delimiter is used between each hextet
	 *
	 * @param s
	 * @param delimiter
	 * @return
	...
	/**
	 * Returns a new byte array containing the data from the provided byte array starting from the provided <code>start</code> position using
	 * <code>len</code> bytes.
	 *
	 * @param array
	 *          The original byte array.
	 * @param start
	 *          The start point from where in the original byte array the data should be taken.
	 * @param len
	 *          The amount of bytes which should be taken from the original byte array.
	...
	/**
	 * Returns a new byte array containing the data from the provided byte array starting from the provided <code>start</code> position using
	 * <code>len</code> bytes. If there are not enough bytes remaining in the byte array to create the new byte array but the
	 * <code>getRest</code> switch is enabled the available bytes are used instead.s
	 *
	 * @param array
	 *          The original byte array.
	 * @param start
	 ...
	 *          The amount of bytes which should be taken from the original byte array.
	 * @param getRest
	 *          Switch which indicates if we should use the data from the byte array even if there aren't enough bytes as specified by
	 *          <code>len</code>
	 * @return The resulting byte array.
	 */
	...
	/**
	 * Convertes the provided byte array to an integer value. The 4 bytes needed for the integer transformation are taken from the byte array
	...
	/**
	 * Converts a short value into the appropriate sequence of byte values. The result is returned in a byte array containing two values.
	 *
	 * @param shortValue
	...
	/**
	 * Converts a integer value into the appropriate sequence of byte values. The result is returned in a byte array containing four values.
	 *
	 * @param intValue
	...
	/**
	 * Primitive function which converts a signed byte into an unsigned integer. Therefore, converts the singed value into an unsigned value.
	 * This can also be casted into a short value without lost of precision.
	 *
	 * @param b
	...
	/**
	 * Converts an MAC (XX:XX:XX:XX:XX:XX) or IP address (X[XX].X[XX].X[XX].X[XX]) into a corresponding byte array. The returned byte array is
	 * therefore either 6 (MAC) or 4 (IP) bytes long.
	 *
	 * @param s
	 *          The MAC or IP address as <code>String</code>.
	 * @param delimiter
	 *          The delimiter character, either <code>:</code> or <code>.</code>.
	...
	/**
	 * Checks if the provided <code>String</code> is in HEX format. This is a very simple and failure prone check, since this function only
	 * searches the <code>String</code> for occurences of possible HEX characters. It does not verify if it can be converted into a decimal
	 * number.
	 *
	 * @param s
	...
	/**
	 * Concatenates two byte arrays and returns the newly created byte array containing the content of both. The content of the byte array
	 * data1 is on the lower indexes and the content of the byte array data2 is on the higher indexes of the returned byte array.
	 *
	 * @param data1
	 *          Byte array which should come first in the new combined byte array.
	 * @param data2
	 *          Byte array which should come first in the new combined byte array.
	...
	/**
	 * Inserts the content of the byte array <code>data2</code> at the specified offset position into the byte array <code>data</code>. This
	 * function only is executed if the provided content of the byte array <code>data2</code> fits into the byte array <code>data</code>.
	 *
	 * @param data
	...
	/**
	 * Calculates the correct Checksum. The checksum is calculated over the content of the provided packet data.
	 *
	 * @return The checksum as short value.
	 */
	public static short calculateChecksum(byte[] packet) {
		int i;
		int ofs = 0;
		int sum = 0;
		byte cnt = (byte) (packet.length / 4);

		// TODO: Maybe we can change this to handle byte instead of
		// converting the data first to integers. Maybe faster

		while (cnt != 0) {
			i = NetUtils.bytesToInt(packet, ofs);
			sum += i & 0xffff;
			sum += i >>> 16;
			ofs = ofs + 4;
			--cnt;
		...
		int modulo = packet.length % 4;
		if (modulo != 0) {
			i = NetUtils.bytesToInt(packet, ofs, true);
			switch (modulo) {
			case 1:
				sum += i >>> 16 & 0xFF00;
				break;
	...
	public static boolean isUseableIPv4Address(String address) {
		try {
