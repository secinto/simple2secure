...
public class DeviceController {
	...
	@RequestMapping(
			...
			method = RequestMethod.POST)
	@PreAuthorize("hasAnyAuthority('DEVICE')")
	public ResponseEntity<Service> getServiceVersion(@RequestBody Device device, @RequestHeader("Accept-Language") String locale) {
		if (device != null && !Strings.isNullOrEmpty(device.getDeviceId())) {
			CompanyLicensePrivate license = licenseRepository.findByDeviceId(device.getDeviceId());
			license.setLastOnlineTimestamp(System.currentTimeMillis());
