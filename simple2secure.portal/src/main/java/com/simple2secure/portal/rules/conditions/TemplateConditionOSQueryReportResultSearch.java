...
@AnnotationCondition(
		name_tag = "os_query_report_condition_name_result_parser",
		description_tag = "os_query_report_condition_description_result_parser",
		fact_type = RuleFactType.OSQUERYREPORT)
public class TemplateConditionOSQueryReportResultSearch extends AbstractPortalCondition<OsQueryReport>
{
	@Autowired
	...
	protected boolean condition(OsQueryReport osQueryReport, ObjectId contextId) {
		...
		JsonNode resultEntries = JSONUtils.fromString(osQueryReport.getQueryResult());
		
		for(JsonNode entry : resultEntries)
		{
			result = entry.toString().matches(regex);
			if(result)
				break;
		}
